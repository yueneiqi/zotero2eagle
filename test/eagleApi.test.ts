...
describe("Eagle API", function () {
  describe("Static methods", function () {
    ...
    it("should generate Zotero item URL correctly", function () {
      const url = EagleApi.generateZoteroItemUrl(
        "ABC123",
        "5",
        "annotation456",
      );
  ...
  describe("Network connectivity (if Eagle is running)", function () {
    ...
    it("should fail with invalid URL", async function () {
      ...
      const result = await EagleApi.addItemFromURL(
        "http://locahost_invalid:442134",
        "",
        invalidItem,
      );
      assert.equal(
      ...
      try {
        ...
        if (result.status === "success") {
          ...
          assert.equal(
            result.status,
            "error",
            "Should return error status if Eagle unavailable",
          );
    ...
    it.skip("should add item from path with valid parameters", async function () {
      ...
      try {
        const result = await EagleApi.addItemFromPath(
          baseUrl,
          apiToken,
          testItem,
        );
    ...
    it("should handle authentication error in addItemFromPath", async function () {
      ...
      assert.equal(
        result.status,
        "error",
        "Should return error status for invalid token",
      );
      assert.include(
        result.message,
        "Request timed out",
