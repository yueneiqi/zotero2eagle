...
目前仅在 macOS 13.7.8 上完成测试，其他操作系统可能需要额外验证。

## 免责声明

> **⚠️ 免责声明**: 本项目由 Vibe Coding 开发，以现状提供，使用风险请自行承担。
...
3. 点击“测试连接”以验证设置
   - **已知问题**：即使成功连接 Eagle，`测试连接` 按钮也可能不会显示成功状态；请尝试创建图片标注来确认连接是否可用。
...
![演示](../doc/data/demo.gif)

1. 在 Zotero 的 PDF 阅读器中打开一个 PDF
2. 创建图片标注（高亮并提取图片区域）
3. 图片会自动处理并保存到 Eagle
4. 可选：如果配置了输出目录，也会在本地保存
5. 完成eagle的导入后，可点击图片url的回链快速跳转到zotero pdf的位置
